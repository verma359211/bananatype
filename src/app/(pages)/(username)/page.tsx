			...
			<CardContent className="pt-6">
				<div className="flex flex-col items-center">
					...
					<p className="mt-2 text-center">
						Last Online:{" "}
						{user?.history?.[user.history.length - 1]?.testPlayed
							? new Date(
									user.history[user.history.length - 1].testPlayed
							  ).toLocaleString()
							: "No data available"}
				...
				<div className="space-y-2">
					<div className="flex justify-between">
						<span className="flex items-center">
							<Target className="mr-2" /> Tests Attempted
						</span>
						<span className="font-semibold">{user?.testAttempted || 0}</span>
					</div>
					<div className="flex justify-between">
						<span className="flex items-center">
							<Trophy className="mr-2" /> Top Speed
						</span>
						<span className="font-semibold">{user?.topSpeed || 0} WPM</span>
					</div>
					<div className="flex justify-between">
						<span className="flex items-center">
							<Activity className="mr-2" /> Average Speed
						</span>
						<span className="font-semibold">
							{(user?.avgSpeed || 0).toFixed(2).slice(0, 2)} WPM
						</span>
					</div>
				</div>
			</CardContent>
							...
							<div key={test._id} className="flex justify-between items-center">
								<div>
									...
									<p className="text-sm text-muted-foreground">
										Accuracy:{" "}
										{test.accuracy ? test.accuracy.toFixed(2) : "N/A%"}%
									</p>
								</div>
								<p className="text-sm text-muted-foreground">
